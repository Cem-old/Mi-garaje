...
const KEY='mi_garaje_public_demo_v1',EMPTY_DB={schemaVersion:8.1,vehicles:[],workshop:[],tires:[],itv:[],taxes:[],other:[],insuranceHistory:[],insurancePolicies:[]};
...
function itvDue(v){if(v.type==='Bici'||v.status==='Histórico')return null;let l=latest(db.itv,v.id);if(l?.nextDate)return pd(l.nextDate);let f=pd(v.firstRegistration);return f?ay(f,4):null}function stat(d){if(!d)return ['Pendiente','bad'];let n=dy(d);return n<0?['Vencida','bad']:n<=30?[n+' días','warn']:[n+' días','good']}
function bottom(){return `<div class=copyright>© Cem 2026 · V8.1</div><div class=bottom>${[['garage','⌂','Garaje'],['agenda','▣','Agenda'],['spend','▥','Gastos'],['more','•••','Más']].map(n=>`<div class="nav ${state.page===n[0]?'on':''}" onclick="go('${n[0]}')"><b>${n[1]}</b>${n[2]}</div>`).join('')}</div>`}function go(p){state.page=p;render()}
function welcome(){
...
function vehicleCard(v){
  ...
  let i=historical?['Histórico','']:stat(itvDue(v)),sd=historical?null:(v.insurance?stat(pd(v.insurance.renewal)):null),t=latest(db.tires,v.id),tk=t&&v.km&&t.km?Math.max(0,+v.km-+t.km):null;
  let ms=historical?
    [['✓','Vendido',v.saleDate||'Histórico'],['€','Importe',v.salePrice!==''&&v.salePrice!=null?money(v.salePrice):'—'],['→','Destinatario',v.saleRecipient||'—']]:
    v.type==='Bici'?[['🔧','Mantenimiento',latest(db.workshop,v.id)?.date||'Sin datos'],['◉','Neumáticos',tk!=null?tk.toLocaleString('es-ES')+' km':'Sin km'],['','', '']]:[['▣','ITV',i[0]],['⬟','Seguro',sd?sd[0]:'Pendiente'],['◉','Neumáticos',tk!=null?tk.toLocaleString('es-ES')+' km':'Sin km']];
  return `<div class=vehicle onclick="openV('${v.id}')"><div class=vpic>${v.photo?`<img src="${v.photo}" style="width:100%;height:100%;object-fit:cover">`:v.type==='Bici'?'🚲':v.type==='Moto'?'🏍️':'🚙'}<div class=vtitle><strong>${esc(v.alias)}</strong>${v.type!=='Bici'?`<span class="plate editable" onclick="event.stopPropagation();state.vid='${v.id}';state.page='vehicle';render();setTimeout(()=>editVehicleField('plate'),0)">${v.plate?esc(v.plate):'＋ Añadir matrícula'}</span>`:''}</div></div><div class=metrics>${ms.map((m,j)=>m[0]?`<div class=metric><span>${m[0]} ${m[1]}</span><b class="${!historical&&j<2&&v.type!=='Bici'?(j===0?i[1]:sd?.[1]||'bad'):''}">${esc(m[2])}</b></div>`:'<div></div>').join('')}</div></div>`
}
...
function addVehicle(){
  ...
  let id=uid('veh');
  db.vehicles.push({id,alias:String(alias).trim(),brand:String(brand).trim(),model:String(model).trim(),type,status:'Activo',plate:String(plate).trim(),vin:String(vin).trim(),purchase,firstRegistration,purchasePrice:price,km:km===''?'':km,mileageUpdated:km!==''?todayES():'',insurance:null,mileageHistory:[],saleDate:'',salePrice:'',saleRecipient:''});
...
function editVehicle(){
  ...
  Object.assign(v,{alias:alias.trim(),brand:brand.trim(),model:model.trim(),purchase,purchasePrice:price,plate:String(plate).trim(),firstRegistration:first,vin:vin.trim()});
  save();render()
}
...
function vehicle(){
  let v=V(),historical=v.status==='Histórico',i=stat(itvDue(v)),sd=!historical&&v.insurance?stat(pd(v.insurance.renewal)):null,t=latest(db.tires,v.id),tk=t&&v.km&&t.km?Math.max(0,+v.km-+t.km):null,w=latest(db.workshop,v.id),tx=latest(db.taxes||[],v.id);
  ...
  if(v.type!=='Bici'){
    ...
    cards.push(['insurance','Seguro',historical?'Historial':(sd?sd[0]:'Pendiente'),historical?'Consultar pólizas y primas':(v.insurance?.company||v.insurance?.renewal||'Completar datos')]);
  }
  cards.push(['tires','Neumáticos',tk!=null?tk.toLocaleString('es-ES')+' km':'—',t?'Último montaje '+t.date:'Sin datos']);
  cards.push(['workshop','Taller',w?.date||'Sin datos',w?.description||'Mantenimiento y reparaciones']);
  if(v.type!=='Bici')cards.push(['taxes','Impuestos',tx?.date||'Sin datos',tx?.cost!==''&&tx?.cost!=null?money(tx.cost):'Historial de pagos']);
  cards.push(['other','Gastos','Ver historial','Accesorios y otros']);
  app().innerHTML=`<div class=top><button class=back onclick="go('garage')">‹ Mi Garaje</button></div><main><div class=hero><div class=heroimg>${v.photo?`<img src="${v.photo}">`:v.type==='Bici'?'🚲':v.type==='Moto'?'🏍️':'🚙'}<div class=herotxt><h1>${esc(v.alias)}</h1>${v.plate?`<span class=plate>${esc(v.plate)}</span>`:''}</div><button class=photobtn onclick=pickPhoto()>＋ Foto</button><input id=photo type=file accept="image/*" hidden onchange=setPhoto(this.files[0])></div><div class="dashboard navdashboard">${cards.map(c=>`<div class="dash navdash" onclick="openM('${c[0]}')"><span class=dashlabel>${c[1]}</span><b>${esc(c[2])}</b><small>${esc(c[3])}</small><span class=dashchev>›</span></div>`).join('')}<div class="dash navdash mileage" onclick=updateMileage()><span class=dashlabel>Kilometraje</span><b>${v.km!==''&&v.km!=null?Number(v.km).toLocaleString('es-ES')+' km':'—'}</b><small>${v.mileageUpdated?'Actualizado '+esc(v.mileageUpdated):'Toca para actualizar'}</small><span class=dashchev>›</span></div></div></div>${sale}<div class=section>Información del vehículo</div><div class="card infocard">${(v.type==='Bici'?[['Marca',v.brand],['Modelo',v.model],['Fecha de compra',v.purchase||'—'],['Precio de compra',v.purchasePrice!==''&&v.purchasePrice!=null?money(v.purchasePrice):'—'],['Bastidor',v.vin||'—']]:[['Marca',v.brand],['Modelo',v.model],['Matrícula',v.plate||'—'],['Fecha de compra',v.purchase||'—'],['Precio de compra',v.purchasePrice!==''&&v.purchasePrice!=null?money(v.purchasePrice):'—'],['Primera matriculación',v.firstRegistration||'—'],['Bastidor',v.vin||'—']]).map(r=>`<div class=row><span class=muted>${r[0]}</span><b>${esc(r[1])}</b></div>`).join('')}</div><button class="add secondary fullbtn" onclick=editVehicle()>Editar ficha completa</button>${historical?'':`<button class="saleaction" onclick=sellVehicle()>Registrar venta</button>`}</main>${bottom()}`
}
...
function setPhoto(f){
  ...
  r.onload=()=>{let img=new Image();img.onload=()=>{
    ...
    let previous=V().photo;V().photo=data;
    if(!save()){V().photo=previous;alert('La foto se ha reducido, pero el espacio reservado por iOS sigue siendo insuficiente. Conserva tu copia JSON y prueba eliminando/reemplazando alguna foto muy antigua.');return}
    render()
...
function excelRows(){
  let vehicles=(db.vehicles||[]).map(v=>({id:v.id,Alias:v.alias||'',Tipo:v.type||'',Estado:v.status||'',Marca:v.brand||'',Modelo:v.model||'',Matricula:v.plate||'',Bastidor:v.vin||'',PrimeraMatriculacion:v.firstRegistration||'',FechaCompra:v.purchase||'',PrecioCompra:v.purchasePrice??'',KmActuales:v.km??'',FechaKm:v.mileageUpdated||'',PlanMantenimientoMensual:v.maintenancePlan?.amount??'',DiaCargoMantenimiento:v.maintenancePlan?.day??'',DescripcionPlanMantenimiento:v.maintenancePlan?.label||'',Foto:v.photo?'SI':''}));
...
function importExcel(file){
  if(!file||!xlsxReady())return;let r=new FileReader();r.onload=()=>{try{
    ...
    downloadBackup('antes_de_importar_excel');let previous=structuredClone(db),incoming=structuredClone(EMPTY_DB);incoming.schemaVersion=8.1;
    incoming.vehicles=vr.filter(x=>x.id||x.Alias).map(row=>{let id=String(row.id||uid('veh'));let local=(db.vehicles||[]).find(v=>v.id===id);return {id,alias:row.Alias||'',type:row.Tipo||local?.type||'Otro',status:row.Estado||local?.status||'Activo',brand:row.Marca||'',model:row.Modelo||'',plate:row.Matricula||'',vin:row.Bastidor||'',firstRegistration:xdate(row.PrimeraMatriculacion),purchase:xdate(row.FechaCompra),purchasePrice:nval(row.PrecioCompra),km:nval(row.KmActuales),mileageUpdated:xdate(row.FechaKm),photo:local?.photo||'',insurance:local?.insurance||null,mileageHistory:local?.mileageHistory||[],saleDate:local?.saleDate||'',salePrice:local?.salePrice??'',saleRecipient:local?.saleRecipient||'',maintenancePlan:(row.PlanMantenimientoMensual!==''&&row.PlanMantenimientoMensual!=null)?{amount:nval(row.PlanMantenimientoMensual),day:Math.min(28,Math.max(1,Math.round(nval(row.DiaCargoMantenimiento)||1))),label:row.DescripcionPlanMantenimiento||'Cuota mensual de mantenimiento'}:(local?.maintenancePlan||null)}}
    );
...
function normalizeData(data){
  ...
  data.vehicles.forEach(v=>{v.mileageHistory=Array.isArray(v.mileageHistory)?v.mileageHistory:[];v.saleDate=v.saleDate||'';v.salePrice=v.salePrice??'';v.saleRecipient=v.saleRecipient||'';if(v.maintenancePlan&&typeof v.maintenancePlan!=='object')v.maintenancePlan=null});
  data.schemaVersion=Math.max(Number(data.schemaVersion)||0,8.1);
  return data
}
...
function mergeData(incoming,local){
  ...
  return {schemaVersion:Math.max(Number(incoming.schemaVersion)||0,Number(local.schemaVersion)||0,8.1),
    vehicles,
...
function mergeDataExternal(incoming,local){incoming=normalizeData(incoming);local=normalizeData(local);let im=new Map(incoming.vehicles.map(v=>[v.id,v])),lm=new Map(local.vehicles.map(v=>[v.id,v])),ids=new Set([...lm.keys(),...im.keys()]);let vehicles=[...ids].map(id=>{let l=lm.get(id),i=im.get(id);if(!i)return l;if(!l)return i;return {...l,...i,photo:l.photo||i.photo||'',insurance:l.insurance||i.insurance||null,mileageHistory:i.mileageHistory?.length?i.mileageHistory:(l.mileageHistory||[])}});return {schemaVersion:8.1,vehicles,workshop:mergeArrayExternal(incoming.workshop,local.workshop),tires:mergeArrayExternal(incoming.tires,local.tires),itv:mergeArrayExternal(incoming.itv,local.itv),taxes:mergeArrayExternal(incoming.taxes,local.taxes),other:mergeArrayExternal(incoming.other,local.other),insuranceHistory:mergeArrayExternal(incoming.insuranceHistory,local.insuranceHistory),insurancePolicies:mergeArrayExternal(incoming.insurancePolicies,local.insurancePolicies)}}
function syncLegacyInsurance(){(db.vehicles||[]).forEach(v=>{if(v.status==='Histórico')return;let p=(db.insurancePolicies||[]).filter(x=>x.vehicle===v.id&&x.active).sort((a,b)=>dateValue(b.start)-dateValue(a.start))[0];if(p)v.insurance={company:p.company||'',policy:p.policy||'',holder:p.holder||'',additional:p.additional||'',coverage:p.coverage||'',start:p.start||'',renewal:p.renewal||'',bank:p.bank||''}})}
...
function restoreBackup(file){if(!file)return;let r=new FileReader();r.onload=()=>{try{let data=JSON.parse(r.result);if(!data||!Array.isArray(data.vehicles))throw new Error('Copia JSON no válida');if(!confirm('ATENCIÓN: se hará primero una copia de seguridad y después este JSON sustituirá todos los datos actuales de Mi Garaje. ¿Continuar?'))return;downloadBackup('antes_de_restaurar');let previous=db;db=normalizeData(data);if(!save()){db=previous;return}alert('Copia restaurada correctamente.');state.page='garage';state.filter='Moto';render()}catch(e){console.error(e);alert('No se ha podido restaurar la copia: '+(e?.message||'archivo no válido'))}};r.readAsText(file)}
