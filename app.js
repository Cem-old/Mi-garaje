...
const KEY='mi_garaje_public_demo_v1',EMPTY_DB={schemaVersion:8.2,vehicles:[],workshop:[],tires:[],itv:[],taxes:[],other:[],insuranceHistory:[],insurancePolicies:[]};
...
function importExcel(file){
  if(!file||!xlsxReady())return;let r=new FileReader();r.onload=async()=>{try{
    ...
    await downloadBackup('antes_de_importar_excel');let previous=structuredClone(db),incoming=structuredClone(EMPTY_DB);incoming.schemaVersion=8.2;
...
function normalizeData(data){
  ...
  data.vehicles.forEach(v=>{v.mileageHistory=Array.isArray(v.mileageHistory)?v.mileageHistory:[];v.saleDate=v.saleDate||'';v.salePrice=v.salePrice??'';v.saleRecipient=v.saleRecipient||'';v.owner=v.owner||'';v.fuel=v.fuel||'';v.displacement=v.displacement??'';v.environmentalLabel=v.environmentalLabel||'';v.nive=v.nive||'';if(v.maintenancePlan&&typeof v.maintenancePlan!=='object')v.maintenancePlan=null});
  data.schemaVersion=Math.max(Number(data.schemaVersion)||0,8.2);
  return data
}
...
function mergeData(incoming,local){
  ...
  return {schemaVersion:Math.max(Number(incoming.schemaVersion)||0,Number(local.schemaVersion)||0,8.2),
    vehicles,
...
async function fullBackupData(){let out=structuredClone(db);for(const v of out.vehicles||[]){let p=photoCache.get(v.id)||'';if(!p&&photoStorageReady)p=await photoGet(v.id).catch(()=>'');v.photo=p||v.photo||''}out.schemaVersion=8.2;return out}
...
function render(){({garage,vehicle,workshop,tires,insurance,itv,taxes,docs,other,agenda,spend,more,pending}[state.page]||garage)()}
