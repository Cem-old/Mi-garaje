...
const KEY='mi_garaje_public_demo_v1',EMPTY_DB={schemaVersion:8.3,vehicles:[],workshop:[],tires:[],itv:[],taxes:[],other:[],insuranceHistory:[],insurancePolicies:[]};
...
function bottom(){return `<div class=copyright>© Cem 2026 · V8.3</div><div class=bottom>${[['garage','⌂','Garaje'],['agenda','▣','Agenda'],['spend','▥','Gastos'],['more','•••','Más']].map(n=>`<div class="nav ${state.page===n[0]?'on':''}" onclick="go('${n[0]}')"><b>${n[1]}</b>${n[2]}</div>`).join('')}</div>`}function go(p){state.page=p;render()}
function welcome(){
...
function vehicleCard(v){
  ...
  let ms=historical?
    ...
    v.type==='Bici'?[['🔧','Mantenimiento',latest(db.workshop,v.id)?.date||'Sin datos'],['◉','Neumáticos',tk!=null?tk.toLocaleString('es-ES')+' km':'Sin km'],['','', '']]:[['▣','ITV',i[0]],['⬟','Seguro',cp?(sd?sd[0]:'Vigente'):'Sin seguro'],['◉','Neumáticos',tk!=null?tk.toLocaleString('es-ES')+' km':'Sin km']];
...
function importExcel(file){
  if(!file||!xlsxReady())return;let r=new FileReader();r.onload=async()=>{try{
    ...
    await downloadBackup('antes_de_importar_excel');let previous=structuredClone(db),incoming=structuredClone(EMPTY_DB);incoming.schemaVersion=8.3;
...
function normalizeData(data){
  ...
  data.schemaVersion=Math.max(Number(data.schemaVersion)||0,8.3);
  return data
}
...
function mergeData(incoming,local){
  ...
  return {schemaVersion:Math.max(Number(incoming.schemaVersion)||0,Number(local.schemaVersion)||0,8.3),
    vehicles,
...
async function fullBackupData(){let out=structuredClone(db);for(const v of out.vehicles||[]){let p=photoCache.get(v.id)||'';if(!p&&photoStorageReady)p=await photoGet(v.id).catch(()=>'');v.photo=p||v.photo||''}out.schemaVersion=8.3;return out}
