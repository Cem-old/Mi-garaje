...
const KEY='mi_garaje_public_demo_v1',EMPTY_DB={schemaVersion:8.4,vehicles:[],workshop:[],tires:[],itv:[],taxes:[],other:[],insuranceHistory:[],insurancePolicies:[]};
...
const purchaseCenturyFixes=fixFuturePurchaseCentury(db);if(purchaseCenturyFixes)save();
...
let state={page:'garage',filter:'Moto',vid:null,module:null};const app=()=>document.getElementById('app');
const FACE_ENABLED_KEY='mi_garaje_faceid_enabled_v1',FACE_CRED_KEY='mi_garaje_faceid_credential_v1';
let authUnlocked=localStorage.getItem(FACE_ENABLED_KEY)!=='1',hiddenAt=0;
function faceIdEnabled(){return localStorage.getItem(FACE_ENABLED_KEY)==='1'}
function faceIdSupported(){return !!(window.PublicKeyCredential&&navigator.credentials&&window.isSecureContext)}
function rnd(n=32){let a=new Uint8Array(n);crypto.getRandomValues(a);return a}
...
function fromB64url(s){s=s.replace(/-/g,'+').replace(/_/g,'/');while(s.length%4)s+='=';let bin=atob(s),a=new Uint8Array(bin.length);for(let i=0;i<bin.length;i++)a[i]=bin.charCodeAt(i);return a}
async function enableFaceId(){
  if(!faceIdSupported()){alert('Face ID no está disponible aquí. Abre Mi Garaje desde Safari o la app instalada mediante HTTPS.');return}
  try{
    let cred=await navigator.credentials.create({publicKey:{
      challenge:rnd(),rp:{name:'Mi Garaje'},user:{id:rnd(24),name:'mi-garaje-local',displayName:'Mi Garaje'},
      pubKeyCredParams:[{type:'public-key',alg:-7},{type:'public-key',alg:-257}],
      authenticatorSelection:{authenticatorAttachment:'platform',residentKey:'preferred',userVerification:'required'},
      timeout:60000,attestation:'none'
    }});
    if(!cred)throw new Error('No se creó la credencial');
    localStorage.setItem(FACE_CRED_KEY,b64url(cred.rawId));
    localStorage.setItem(FACE_ENABLED_KEY,'1');
    authUnlocked=true;alert('Protección con Face ID activada. Mi Garaje no guarda ninguna imagen ni dato biométrico.');render()
  }catch(e){if(e?.name!=='NotAllowedError')console.error(e);alert('No se ha podido activar Face ID. No se ha cambiado la protección de la app.')}
}
function disableFaceId(){
  if(!faceIdEnabled())return;
  if(!confirm('¿Desactivar la protección con Face ID?'))return;
  localStorage.removeItem(FACE_ENABLED_KEY);localStorage.removeItem(FACE_CRED_KEY);authUnlocked=true;render()
}
function lockNow(){if(!faceIdEnabled())return;authUnlocked=false;render()}
async function unlockFaceId(){
  if(!faceIdEnabled()){authUnlocked=true;render();return}
  if(!faceIdSupported()){alert('No se puede usar Face ID en este contexto. Abre Mi Garaje mediante HTTPS.');return}
  let id=localStorage.getItem(FACE_CRED_KEY);if(!id){alert('No encuentro la credencial de Face ID. Desactiva y vuelve a activar la protección desde Más.');return}
  try{
    let assertion=await navigator.credentials.get({publicKey:{challenge:rnd(),allowCredentials:[{type:'public-key',id:fromB64url(id)}],userVerification:'required',timeout:60000}});
    if(assertion){authUnlocked=true;render()}
  }catch(e){if(e?.name!=='NotAllowedError')console.error(e)}
}
function renderLockScreen(){
  app().innerHTML=`<main class=lockscreen><div class=lockcard><div class=lockicon>◉</div><h1>Mi Garaje</h1><p>Protegido con Face ID</p><button class=add onclick=unlockFaceId()>Desbloquear con Face ID</button><small>La biometría permanece en el iPhone. Mi Garaje solo recibe la confirmación del sistema.</small></div></main>`;
}
document.addEventListener('visibilitychange',()=>{if(document.hidden){hiddenAt=Date.now()}else if(faceIdEnabled()&&hiddenAt&&Date.now()-hiddenAt>30000){authUnlocked=false;render()}});
const esc=x=>String(x??'').replace(/[&<>"]/g,c=>({'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;'}[c]));
function expandYear(y){y=Number(y);if(!Number.isFinite(y))return NaN;return y<100?(y>=50?1900+y:2000+y):y}function pd(s){if(!s)return null;s=String(s).trim();if(!s||s.startsWith('Año'))return null;let m=s.match(/^(\d{1,2})[\/.-](\d{1,2})[\/.-](\d{2,4})$/),d,mo,y;if(m){d=Number(m[1]);mo=Number(m[2]);y=expandYear(m[3])}else{m=s.match(/^(\d{4})-(\d{1,2})-(\d{1,2})$/);if(!m)return null;y=Number(m[1]);mo=Number(m[2]);d=Number(m[3])}if(!Number.isFinite(y)||mo<1||mo>12||d<1||d>31)return null;let x=new Date(y,mo-1,d);return x.getFullYear()===y&&x.getMonth()===mo-1&&x.getDate()===d?x:null}function fd(d){return d?String(d.getDate()).padStart(2,'0')+'/'+String(d.getMonth()+1).padStart(2,'0')+'/'+d.getFullYear():''}function fixFuturePurchaseCentury(data){let nowY=new Date().getFullYear(),changed=0;(data?.vehicles||[]).forEach(v=>{let m=String(v.purchase||'').trim().match(/^(\d{1,2})[\/.-](\d{1,2})[\/.-](\d{4})$/);if(!m)return;let y=Number(m[3]);if(y>=2050&&y<=2099&&y>nowY+1){let corrected=new Date(y-100,Number(m[2])-1,Number(m[1]));v.purchase=fd(corrected);changed++}});return changed}function dy(d){return Math.ceil((d-new Date())/86400000)}function ay(d,n){let x=new Date(d);x.setFullYear(x.getFullYear()+n);return x}function V(){return db.vehicles.find(x=>x.id===state.vid)}function latest(a,id){return a.filter(x=>x.vehicle===id&&pd(x.date)).sort((x,y)=>pd(y.date)-pd(x.date))[0]}
...
function workshopKmSince(w,v){return w&&hasNum(w.km)&&hasNum(v?.km)?Math.max(0,Number(v.km)-Number(w.km)):null}
...
function bottom(){return `<div class=copyright>© Cem 2026 · V9.0</div><div class=bottom>${[['garage','⌂','Garaje'],['agenda','▣','Agenda'],['spend','▥','Gastos'],['more','•••','Más']].map(n=>`<div class="nav ${state.page===n[0]?'on':''}" onclick="go('${n[0]}')"><b>${n[1]}</b>${n[2]}</div>`).join('')}</div>`}function go(p){state.page=p;render()}
function welcome(){
...
function vehicleCard(v){
  ...
  let ms=historical?
    [['↦','Desde',v.purchase||'—'],['↤','Hasta',v.saleDate||'—'],['','','']]:
...
function addVehicle(){
  ...
  let alias=prompt('Alias / nombre para identificarlo:','');if(alias===null||!String(alias).trim())return;alias=String(alias).trim();if(aliasTaken(alias)){alert('Ya existe un vehículo con ese alias. El alias debe ser único porque también será su ID en Excel.');return}
  ...
  if(type!=='Otros'){
    ...
    displacement=promptNumber('Cilindrada (cc, opcional)','','km');if(displacement===null||displacement===undefined)return;
  ...
  let km=promptNumber('Kilómetros actuales','','km');if(km===null||km===undefined)return;
...
function aliasKey(s){return String(s||'').trim().toLocaleLowerCase('es-ES')}
function aliasTaken(alias,exceptId=''){let k=aliasKey(alias);return (db.vehicles||[]).some(v=>v.id!==exceptId&&aliasKey(v.alias)===k)}
async function migrateVehicleIdsToAliases(){
  let vs=db.vehicles||[],keys=vs.map(v=>aliasKey(v.alias));
  if(!vs.length||keys.some(k=>!k)||new Set(keys).size!==keys.length)return false;
  let map=new Map(vs.filter(v=>v.id!==String(v.alias).trim()).map(v=>[v.id,String(v.alias).trim()]));
  if(!map.size)return false;
  // First copy photos to the new keys; only then change relational IDs.
  if(photoStorageReady){
    for(const [oldId,newId] of map){
      let p=photoCache.get(oldId)||await photoGet(oldId).catch(()=>'');
      if(p){await photoPut(newId,p);photoCache.set(newId,p)}
    }
  }
  vs.forEach(v=>{if(map.has(v.id))v.id=map.get(v.id)});
  for(const name of ['workshop','tires','itv','taxes','other','insuranceHistory','insurancePolicies'])
    (db[name]||[]).forEach(x=>{if(map.has(x.vehicle))x.vehicle=map.get(x.vehicle)});
  if(photoStorageReady){
    for(const [oldId] of map){await photoDelete(oldId).catch(()=>{});photoCache.delete(oldId)}
  }
  save();return true
}
...
function promptDate(label,value=''){let v=prompt(label+' (dd/mm/aaaa):',value||'');if(v===null)return null;v=String(v).trim();if(v==='')return '';let d=pd(v);if(!d){alert('Fecha no válida');return undefined}return fd(d)}
function parseLocaleNumber(v,kind='amount'){
  ...
  let dots=(s.match(/\./g)||[]).length,commas=(s.match(/,/g)||[]).length;
  if(kind==='km'){
    if(dots&&commas){
      let dec=s.lastIndexOf(',')>s.lastIndexOf('.')?',':'.',th=dec===','?'.':',';
    ...
    if(dots&&commas){
      ...
      let p=s.split(',');
      if(p.length===2)s=p[0]+'.'+p[1];else{s=p.slice(0,-1).join('')+'.'+p.at(-1)}
    }else if(dots){
      let p=s.split('.');
      if(p.length===2&&p[1].length<=2)s=p[0]+'.'+p[1];
      else if(p.every((x,i)=>i===0||x.length===3))s=p.join('');
      else if(p.length>2)s=p.slice(0,-1).join('')+'.'+p.at(-1);
    }
  }
  let n=Number((neg?'-':'')+s);return Number.isFinite(n)?n:NaN
}
function promptNumber(label,value='',kind='amount'){let v=prompt(label+':',value??'');if(v===null)return null;v=String(v).trim();if(v==='')return '';let n=parseLocaleNumber(v,kind);if(!Number.isFinite(n)){alert((kind==='km'?'Kilometraje':'Importe')+' no válido');return undefined}return n}
...
async function editVehicle(){
  let v=V();if(!v)return;
  let alias=prompt('Alias:',v.alias||'');if(alias===null)return;alias=String(alias).trim();if(!alias){alert('El alias no puede quedar vacío.');return}if(aliasTaken(alias,v.id)){alert('Ya existe otro vehículo con ese alias. El alias debe ser único.');return}let oldVehicleId=v.id;
  ...
  if(hasRoadDocs(v)){
    ...
    displacement=promptNumber('Cilindrada (cc)',displacement,'km');if(displacement===null||displacement===undefined)return;
  ...
  Object.assign(v,{alias,brand:brand.trim(),model:model.trim(),purchase,purchasePrice:price,plate:String(plate).trim(),firstRegistration:first,vin:vin.trim(),owner:String(owner).trim(),fuel:String(fuel).trim(),displacement:displacement===''?'':displacement,environmentalLabel:String(environmentalLabel).trim(),nive:String(nive).trim()});
  if(oldVehicleId!==alias){
    v.id=alias;
    for(const name of ['workshop','tires','itv','taxes','other','insuranceHistory','insurancePolicies'])(db[name]||[]).forEach(x=>{if(x.vehicle===oldVehicleId)x.vehicle=alias});
    if(photoStorageReady){let p=photoCache.get(oldVehicleId)||await photoGet(oldVehicleId).catch(()=>'');if(p){await photoPut(alias,p);photoCache.set(alias,p);await photoDelete(oldVehicleId).catch(()=>{});photoCache.delete(oldVehicleId)}}
    state.vid=alias;
  }
  save();render()
}
...
function vehicle(){
  ...
  if(hasMaintenance(v)){let wk=workshopKmSince(w,v);cards.push(['workshop','Taller',wk!=null?wk.toLocaleString('es-ES')+' km':(w?.date||'Sin datos'),w?(wk!=null?'Desde '+w.date+' · '+(w.description||'Intervención'):(w.description||'Mantenimiento y reparaciones')):'Mantenimiento y reparaciones']);}
...
function editWorkshop(id,defaultKind='Mantenimiento'){
  ...
  let km=promptNumber('Kilómetros del vehículo en esta intervención',x?.km??V()?.km??'','km');if(km===null||km===undefined)return;
  if(km===''||!Number.isFinite(Number(km))||Number(km)<0){alert('El kilometraje es obligatorio en revisiones, mantenimientos y reparaciones.');return}
...
function eventHtml(x){let used=workshopKmSince(x,V());return `<div class=event data-search="${esc((x.description+' '+x.kind+' '+x.place).toLowerCase())}"><div class=date>${esc(x.date)} ${hasNum(x.km)?`<span class=muted>· ${Number(x.km).toLocaleString('es-ES')} km</span>`:''}${used!=null?`<span class=sincekm> · hace ${used.toLocaleString('es-ES')} km</span>`:''}</div><div class=eventcard><span class=tag>${esc(x.kind)}</span>${x.cost!==''?`<span class=cost>${Number(x.cost).toLocaleString('es-ES',{minimumFractionDigits:2})} €</span>`:''}<b style="display:block;margin-top:9px">${esc(x.place)}</b><ul class=ops>${x.description.split(';').map(o=>`<li>${esc(o.trim())}</li>`).join('')}</ul>${x.recurring?'':`<div class=recordactions><button onclick="editWorkshop('${x.id}')">Editar</button><button class=dangerbtn onclick="deleteRecord('workshop','${x.id}')">Eliminar</button></div>`}</div></div>`}function filterEvents(q){document.querySelectorAll('.event').forEach(x=>x.style.display=x.dataset.search.includes(q.toLowerCase())?'':'none')}
...
function editTire(id){
  ...
  let brand=prompt('Marca:',x?.brand||'');if(brand===null)return;
  let model=prompt('Modelo:',x?.model||x?.description||'');if(model===null)return;
  let place=prompt('Taller / proveedor:',x?.place||'');if(place===null)return;
  let km=promptNumber('Km del vehículo al cambiar',x?.km??v.km??'','km');if(km===null||km===undefined)return;
  if(km===''||km<0){alert('El kilometraje del vehículo es obligatorio.');return}
  let opts=v.type==='Coche'?'Delanteros / Traseros / Todos':'Delantero / Trasero / Ambos';
  let position=prompt('Posición: '+opts,x?.position||'Ambos');if(position===null)return;
  let pos=String(position).toLowerCase(),front=/delant|ambos|todos/.test(pos),rear=/tras|ambos|todos/.test(pos);
  if(!front&&!rear){alert('Indica Delantero, Trasero o Ambos.');return}
  let frontSize=x?.frontSize||'',rearSize=x?.rearSize||'';
  if(front){frontSize=prompt('Medida neumático delantero:',frontSize);if(frontSize===null)return}
  if(rear){rearSize=prompt('Medida neumático trasero:',rearSize);if(rearSize===null)return}
  let cost=promptNumber('Importe (€)',x?.cost??'');if(cost===null||cost===undefined)return;
  let description=[brand.trim(),model.trim()].filter(Boolean).join(' ')||x?.description||'Neumático';
  let rec={id:x?.id||uid('tir'),vehicle:state.vid,date,brand:brand.trim(),model:model.trim(),frontSize:String(frontSize).trim(),rearSize:String(rearSize).trim(),description,place:place.trim(),km,position:String(position).trim(),cost};
...
function tires(){
  ...
  app().innerHTML=moduleHead('◉ Neumáticos')+`<main class=modulepage><div class=card><h3>Kilómetros por neumático</h3><div class=row><span>Delantero${v.type==='Coche'?'s':''}</span><b>${use(f)!=null?use(f).toLocaleString('es-ES')+' km':'—'}</b></div><div class=row><span>Trasero${v.type==='Coche'?'s':''}</span><b>${use(r)!=null?use(r).toLocaleString('es-ES')+' km':'—'}</b></div></div>${rows.map(x=>{let travelled=use(x),name=[x.brand,x.model].filter(Boolean).join(' ')||x.description||'Neumático';return `<div class=card><span class=cost>${money(x.cost)}</span><h3>${esc(name)}</h3><div class=row><span class=muted>Fecha cambio</span><b>${esc(x.date)}</b></div><div class=row><span class=muted>Km vehículo</span><b>${hasNum(x.km)?Number(x.km).toLocaleString('es-ES')+' km':'—'}</b></div><div class=row><span class=muted>Km recorridos</span><b>${travelled!=null?travelled.toLocaleString('es-ES')+' km':'—'}</b></div><div class=row><span class=muted>Taller</span><b>${esc(x.place||'—')}</b></div><div class=row><span class=muted>Posición</span><b>${esc(x.position||'—')}</b></div>${x.frontSize?`<div class=row><span class=muted>Medida delantera</span><b>${esc(x.frontSize)}</b></div>`:''}${x.rearSize?`<div class=row><span class=muted>Medida trasera</span><b>${esc(x.rearSize)}</b></div>`:''}<div class=recordactions><button onclick="editTire('${x.id}')">Editar</button><button class=dangerbtn onclick="deleteRecord('tires','${x.id}')">Eliminar</button></div></div>`}).join('')||'<div class=card>Sin neumáticos registrados.</div>'}</main>${bottom()}`
}
function insurance(){
...
function pending(){
  ...
  db.vehicles.filter(v=>v.status==='Activo').forEach(v=>{
    if(v.km===''||v.km===null||v.km===undefined)items.push([v.id,v.alias,'Kilometraje','Falta indicar el kilometraje actual','km']);
    if(hasRoadDocs(v)){
...
function excelRows(){
  ...
  let tires=(db.tires||[]).map(x=>({id:x.id,vehicleId:x.vehicle,Fecha:x.date||'',Km:x.km??'',Proveedor:x.place||'',Marca:x.brand||'',Modelo:x.model||'',MedidaDelantera:x.frontSize||'',MedidaTrasera:x.rearSize||'',Neumatico:x.description||'',Importe:x.cost??'',Posicion:x.position||''}));
...
function xdate(v){if(v===null||v===undefined||v==='')return '';if(typeof v==='number'&&window.XLSX?.SSF?.parse_date_code){let d=XLSX.SSF.parse_date_code(v);if(d)return String(d.d).padStart(2,'0')+'/'+String(d.m).padStart(2,'0')+'/'+d.y}let s=String(v).trim(),d=pd(s);return d?fd(d):s}
function yn(v){let s=String(v??'').trim().toLowerCase();return s==='si'||s==='sí'||s==='true'||s==='1'||s==='yes'}
function nval(v,kind='amount'){let n=parseLocaleNumber(v,kind);return Number.isFinite(n)?n:''}
...
function resolveVehicleRef(ref){let s=String(ref||'').trim(),v=(db.vehicles||[]).find(x=>x.id===s||aliasKey(x.alias)===aliasKey(s));return v?v.id:s}
function importExcel(file){
  if(!file||!xlsxReady())return;let r=new FileReader();r.onload=async()=>{try{
    ...
    await downloadBackup('antes_de_importar_excel');let previous=structuredClone(db),incoming=structuredClone(EMPTY_DB);incoming.schemaVersion=8.4;
    let excelAliases=vr.filter(x=>x.id||x.Alias).map(row=>String(row.Alias||row.id||'').trim());if(excelAliases.some(a=>!a)||new Set(excelAliases.map(aliasKey)).size!==excelAliases.length)throw new Error('Hay alias vacíos o duplicados en la hoja Vehiculos. Cada alias debe ser único.');incoming.vehicles=vr.filter(x=>x.id||x.Alias).map(row=>{let id=String(row.Alias||row.id).trim();let local=(db.vehicles||[]).find(v=>v.id===id||aliasKey(v.alias)===aliasKey(id));return {id,alias:row.Alias||id,type:row.Tipo||local?.type||'Otro',status:row.Estado||local?.status||'Activo',brand:row.Marca||'',model:row.Modelo||'',owner:row.Titular||local?.owner||'',plate:row.Matricula||'',vin:row.Bastidor||'',nive:row.NIVE||local?.nive||'',firstRegistration:xdate(row.PrimeraMatriculacion),fuel:row.Carburante||local?.fuel||'',displacement:nval(row.Cilindrada,'km')!==''?nval(row.Cilindrada,'km'):(local?.displacement??''),environmentalLabel:row.DistintivoAmbiental||local?.environmentalLabel||'',purchase:xdate(row.FechaCompra),purchasePrice:nval(row.PrecioCompra),km:nval(row.KmActuales,'km'),mileageUpdated:xdate(row.FechaKm),photo:'',insurance:local?.insurance||null,mileageHistory:local?.mileageHistory||[],saleDate:local?.saleDate||'',salePrice:local?.salePrice??'',saleRecipient:local?.saleRecipient||'',maintenancePlan:(row.PlanMantenimientoMensual!==''&&row.PlanMantenimientoMensual!=null)?{amount:nval(row.PlanMantenimientoMensual),day:Math.min(28,Math.max(1,Math.round(nval(row.DiaCargoMantenimiento)||1))),label:row.DescripcionPlanMantenimiento||'Cuota mensual de mantenimiento'}:(local?.maintenancePlan||null)}}
    );
    let sales=sheetObjects(wb,'Ventas');sales.forEach(row=>{let v=incoming.vehicles.find(x=>x.id===resolveVehicleRef(row.vehicleId));if(v){v.saleDate=xdate(row.FechaVenta);v.salePrice=nval(row.ImporteVenta);v.saleRecipient=row.Destinatario||'';if(v.saleDate)v.status='Histórico'}});
    incoming.insurancePolicies=sheetObjects(wb,'Seguros').filter(x=>x.id||x.vehicleId).map(row=>({id:String(row.id||uid('pol')),vehicle:resolveVehicleRef(row.vehicleId),company:row.Compania||'',policy:row.Poliza||'',holder:row.Tomador||'',additional:row.ConductorAdicional||'',coverage:row.Cobertura||'',start:xdate(row.Inicio),renewal:xdate(row.Renovacion),end:xdate(row.Fin),bank:row.Banco||'',active:!xdate(row.Fin)}));
    incoming.insuranceHistory=sheetObjects(wb,'PagosSeguro').filter(x=>x.id||x.vehicleId).map(row=>({id:String(row.id||uid('ins')),vehicle:resolveVehicleRef(row.vehicleId),policyId:row.policyId||'',date:xdate(row.Fecha),cost:nval(row.Importe)}));
    incoming.workshop=sheetObjects(wb,'Taller').filter(x=>x.id||x.vehicleId).map(row=>({id:String(row.id||uid('wrk')),vehicle:resolveVehicleRef(row.vehicleId),date:xdate(row.Fecha),km:nval(row.Km,'km'),place:row.Proveedor||'',description:row.Concepto||'',cost:nval(row.Importe),kind:row.Tipo||''}));
    incoming.tires=sheetObjects(wb,'Neumaticos').filter(x=>x.id||x.vehicleId).map(row=>({id:String(row.id||uid('tir')),vehicle:resolveVehicleRef(row.vehicleId),date:xdate(row.Fecha),km:nval(row.Km,'km'),place:row.Proveedor||'',brand:row.Marca||'',model:row.Modelo||row.Neumatico||'',frontSize:row.MedidaDelantera||'',rearSize:row.MedidaTrasera||'',description:row.Neumatico||[row.Marca,row.Modelo].filter(Boolean).join(' '),cost:nval(row.Importe),position:row.Posicion||''}));
    incoming.itv=sheetObjects(wb,'ITV').filter(x=>x.id||x.vehicleId).map(row=>({id:String(row.id||uid('itv')),vehicle:resolveVehicleRef(row.vehicleId),date:xdate(row.Fecha),nextDate:xdate(row.ProximaITV),result:row.Resultado||'',center:row.Centro||'',km:nval(row.Km,'km'),cost:nval(row.Importe)}));
    incoming.taxes=sheetObjects(wb,'Impuestos').filter(x=>x.id||x.vehicleId).map(row=>({id:String(row.id||uid('tax')),vehicle:resolveVehicleRef(row.vehicleId),date:xdate(row.Fecha),year:row.Ano||'',cost:nval(row.Importe),status:row.Estado||'Pagado'}));
    incoming.other=sheetObjects(wb,'OtrosGastos').filter(x=>x.id||x.vehicleId).map(row=>({id:String(row.id||uid('oth')),vehicle:resolveVehicleRef(row.vehicleId),date:xdate(row.Fecha),kind:row.Tipo||'',description:row.Concepto||'',place:row.Proveedor||'',km:nval(row.Km,'km'),cost:nval(row.Importe),notes:row.Observaciones||''}));
    let kms=sheetObjects(wb,'Kilometraje');incoming.vehicles.forEach(v=>{let local=(db.vehicles||[]).find(x=>x.id===v.id);let rows=kms.filter(x=>String(x.vehicleId)===v.id).map(x=>({id:String(x.id||uid('km')),date:xdate(x.Fecha),km:nval(x.Km,'km')}));v.mileageHistory=rows.length?rows:(local?.mileageHistory||[])});
...
function normalizeData(data){
  ...
  data.tires.forEach(t=>{t.brand=t.brand||'';t.model=t.model||t.description||'';t.frontSize=t.frontSize||'';t.rearSize=t.rearSize||'';});
  ...
  data.schemaVersion=Math.max(Number(data.schemaVersion)||0,8.4);
  return data
}
...
function mergeData(incoming,local){
  ...
  return {schemaVersion:Math.max(Number(incoming.schemaVersion)||0,Number(local.schemaVersion)||0,8.4),
    vehicles,
...
async function fullBackupData(){let out=structuredClone(db);for(const v of out.vehicles||[]){let p=photoCache.get(v.id)||'';if(!p&&photoStorageReady)p=await photoGet(v.id).catch(()=>'');v.photo=p||v.photo||''}out.schemaVersion=8.4;return out}
...
function render(){if(faceIdEnabled()&&!authUnlocked)return renderLockScreen();({garage,vehicle,workshop,tires,insurance,itv,taxes,docs,other,agenda,spend,spendVehicle,more,pending}[state.page]||garage)()}
(async()=>{try{await initPhotoStorage();await migrateVehicleIdsToAliases();render()}catch(e){console.error('Error de arranque Mi Garaje',e);let el=document.getElementById('app');if(el)el.innerHTML='<main style="padding:40px 22px;color:#fff;font-family:-apple-system,BlinkMacSystemFont,Segoe UI,sans-serif"><h2>Mi Garaje no ha podido arrancar</h2><p style="color:#aeb9c0">Recarga la página. Si vuelve a aparecer este mensaje, la aplicación mostrará el error en vez de quedarse en negro.</p><pre style="white-space:pre-wrap;color:#ffb3b3">'+esc(e?.message||String(e))+'</pre></main>'}})();
